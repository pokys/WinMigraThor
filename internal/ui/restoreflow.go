...
func (m RestoreWizardModel) renderAppsStep() string {
	...
	// Count how many have winget IDs
	var withWinget int
	for _, app := range m.appItems {
		if app.WingetID != "" {
			withWinget++
	...
	if withWinget == 0 && len(m.appItems) > 0 {
		sb.WriteString("  " + StyleWarning.Render("⚠ No winget matches found — showing all installed apps for reference.") + "\n")
		sb.WriteString("  " + StyleMuted.Render("Script mode will include apps without IDs as comments only.") + "\n\n")
	} else {
		sb.WriteString("  Select apps to reinstall via winget:\n\n")
	}

	if len(m.appItems) == 0 {
		sb.WriteString("  " + StyleMuted.Render("No apps data found in this backup."))
		return sb.String()
	}
